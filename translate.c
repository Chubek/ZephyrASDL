...
const char *macro_normal_field =
    "#define NORMAL_FIELD(type, name) type name\n\n";

const char *macro_sequence_field =
    "#define SEQUENCE_FIELD(type, name) type *name; int name##_count\n\n";

const char *macro_optional_field =
    "#define OPTIONAL_FIELD(type, name) type name; int name##_present\n\n";
...
void install_consfn(Constructor *con) {
  ...
  for (size_t p = 0; p < con->num_fields; p++) {
    EMIT_DEF("%s_tyy %c%s", con->fields[p]->type_id,
             con->fields[p]->kind == SEQUENCE ? '*' : ' ',
             get_field_name(con->fields[p], p, &buff[0]));
    EMIT_DECL("%s_tyy %c%s", con->fields[p]->type_id,
              con->fields[p]->kind == SEQUENCE ? '*' : ' ',
              get_field_name(con->fields[p], p, &buff[0]));
  ...
  for (size_t p = 0; p < con->num_fields; p++) {
    ...
    EMIT_DEF("p->v.%s.%s = %s;\n", to_lowercase(con->id, &buff[0]), fldname,
             fldname);
  }
